...
parserFactory.registerUrlRule(
    (url) => DOMAINS.includes(new URL(url).hostname),
    () => new HakoParser(),
);

class HakoParser extends Parser {
    // eslint-disable-line no-unused-vars
    constructor() {
        super();
        //Optional Parameters:
        // 3 second of throttle to prevent 403
        this.minimumThrottle = 3000;
    }

    async getChapterUrls(dom) {
        // more than 1 section
        const volumes = dom.querySelectorAll("section.volume-list.at-series");
        const chapters = [];
        for (const volume of volumes) {
            const arcName = volume.querySelector("span.sect-title");
            const chapterLinks = volume.querySelectorAll(
                ".list-chapters.at-series div.chapter-name a",
            );
            for (let i = 0; i < chapterLinks.length; i++) {
                let chapterLink = chapterLinks[i];
                const chapter = util.hyperLinkToChapter(chapterLink);
                // first chapter of new arc
                if (i === 0) {
                    chapter.newArc = arcName?.textContent.trim();
                }
    ...
    extractTitle(dom) {
        return dom.querySelector("span.series-name a")?.textContent?.trim();
    }

    findChapterTitle(dom) {
        return dom.querySelector(".title-top");
    }

    extractAuthor(dom) {
        const authorItem = [...dom.querySelectorAll(".info-item")]
            .find(el => el.querySelector(".info-name")?.textContent.includes("Tác giả"));
        return authorItem?.querySelector(".info-value a")?.textContent?.trim()
            ?? super.extractAuthor(dom);
    }


    findCoverImageUrl(dom) {
        const cover = dom.querySelector(".series-cover .content.img-in-ratio");
        if (!cover) { return "https://docln.sbs/img/nocover.jpg"; } // no cover
        return util.extractUrlFromBackgroundImage(cover);
    }

    findContent(dom) {
        const chapterContentSection = dom.querySelector(".reading-content #chapter-content");
        if (chapterContentSection) {
            const extLinks = chapterContentSection.querySelectorAll("a[target='__blank']");
            for (const link of extLinks) {
                link.remove();
            }

            const navigationSection = dom.querySelector("section.rd-basic_icon");
            if (navigationSection) {
                navigationSection.remove();
            }
            return chapterContentSection;
        }
